function [FC_simR, CC_check] = CBIG_MFMem_rfMRI_nsolver_eul_sto_resLH(parameter,SC,y_FC,FC_mask,Nstate,Tepochlong,TBOLD,ifHighres,TE)

%-----------------------------------------------------------------------------
% [FC_simR, CC_check] = CBIG_MFMem_rfMRI_nsolver_eul_sto_resLH(parameter,SC,y_FC,FC_mask,Nstate,Tepochlong,TBOLD,ifHighres)
% Function to 
...
%
% Note: This is a modified version of the original function from estimation step:
% CBIG_MFMem_rfMRI_nsolver_eul_sto.m
%
% Input: 
%     - parameter:  given model parameter
%     - SC:         structural connectivity matrix
%     - y_FC:       functional connectivity vector (after mask)
%     - FC_mask:    to select uppertragular elements
...
%     - ifHighres:  High time resolution turn on=1/off=0
%     - TE:         Echo time
%
% Output:
...
if ifHighres == 0
    ...
    dt_l = 0.0001; %[s]
    disp('set to high time resolution')
end
