...
def execute():
    ...
    if expInfo['language'] == 'English' and expInfo['protocol'] == 'I':
        ...
        def ProbePrint(trialNum, Dim, Quest, SR):
            ...
            if trialNum <= 9:
                if Dim == 'intrusiveness':
                    print('trial', trial_num, ' |', dimension, '|', question, Space2*empty, '|',                     confirmation_status, SR, '(0-10)')
                elif Dim == 'focus':
                    print('trial', trial_num, ' |', dimension, Space1*empty,'|', question, '|',                       confirmation_status, SR, '(0-10)')
                else:    
                    print('trial', trial_num, ' |', dimension, Space1*empty, '|', question, Space2*empty, '|',       confirmation_status, SR, '(0-10)')                                
            else:
                if Dim == 'intrusiveness':
                    print('trial', trial_num, '|', dimension, '|', question, Space2*empty, '|',                       confirmation_status, SR, '(0-10)')
                elif Dim == 'focus':
        ...
        Txt = visual.TextStim(win, name='instruction', text='default text', font=sans, pos=(0, 0),                                   height=float(.04), wrapWidth=1100, color='black')
        ...
        if encoding:
            ...
            if prevRuns:
                ...
                enc_csvFile = rootLog + '/sub-' + expInfo['ID'] + '_ses-' + expInfo['session'] + task_lab +                   '_run-01.csv'            
        ...
        if ES1:
            ...
            if prevRuns:
                ...
                ES1_csvFile = rootLog + '/sub-' + expInfo['ID'] + '_ses-' + expInfo['session'] + task_lab +                   '_run-01.csv'          
        ...
        if ES1:
            ...
            for idx in ES1_trials:
                ...
                rating_scale = visual.RatingScale(win, scale=None, low=0, high=10, markerStart=5, leftKeys='2',                                   rightKeys='4', acceptKeys='3', labels=Lab,
                                                  tickMarks=['0','10'], tickHeight=1, maxTime=6, markerColor='red', textColor='black', textSize=.75, stretch=2.5, noMouse=True, lineColor='#3355FF', marker='triangle', showValue=False, precision=10, showAccept=False, disappear=True)
        ...
        if retrieval:
            ...
            if prevRuns:
                ...
                ret_csvFile = rootLog + '/sub-' + expInfo['ID'] + '_ses-' + expInfo['session'] + task_lab +                   '_run-01.csv'
        ...
        if retrieval:
            ...
            for idx in retrieval_trials:
                ...
                # print trial info on console
                if trial_num <= 9:
                    print('trial', trial_num, ' |', condition, '| prime:', prime_name[:-4], '| target: ',                 target_name[:-4], '| foils: ', foil1_name[:-4], ' & ', foil2_name[:-4])
                else:
                    print('trial', trial_num, '|', condition, '| prime:', prime_name[:-4], '| target: ',                 target_name[:-4], '| foils: ', foil1_name[:-4], ' & ', foil2_name[:-4])
                ...
                RT = subResp[2]
                respInfo = RetrievalInfo(RT, y_val, target_pos, foil1_pos, foil2_pos, target_name,
                ...
                with open(ret_csvFile, 'a') as a_ret:
                    ...
                    writer.writerow([fixStart, trial_num, condition, 'ON', stimStart-fixStart, '--', '--',
                                     '--', '--', '--', '--', '--', '--', '--', '--', '--'])
                    writer.writerow([stimStart, trial_num, condition, 'OFF', '--', prime_name, target_name,                          foil1_name, foil2_name, left_choice, center_choice, right_choice,
                                     '--', '--', '--', '--'])
                    writer.writerow([keyPressTime, trial_num, condition, 'OFF', '--', '--', '--', '--', '--',                    '--', '--', '--', subChoice, key_ID, accuracy, '--'])
        ...
        if ES2:
            ...
            if prevRuns:
                ...
                ES2_csvFile = rootLog + '/sub-' + expInfo['ID'] + '_ses-' + expInfo['session'] + task_lab +                   '_run-01.csv'
        ...
        if MST1:
            ...
            if prevRuns:
                ...
                mst1_csvFile = rootLog + '/sub-' + expInfo['ID'] + '_ses-' + expInfo['session'] + task_lab +                  '_run-01.csv'
        ...
        if MST1:
            ...
            for idx in mst1_trials:
                ...
                MSTtext = visual.TextStim(win, text='Indoors or Outdoors?', font=sans, name='instruction',               pos=(0, .4), height=float(.04), wrapWidth=1100, color='black')
        ...
        if ES3:
            ...
            if prevRuns:
                ...
                ES3_csvFile = rootLog + '/sub-' + expInfo['ID'] + '_ses-' + expInfo['session'] + task_lab +                   '_run-01.csv'            
        ...
        if MST2:
            ...
            if prevRuns:
                ...
                mst2_csvFile = rootLog + '/sub-' + expInfo['ID'] + '_ses-' + expInfo['session'] + task_lab +                  '_run-01.csv'            
            with open(mst2_csvFile, 'w') as w_mst2:
                writer = csv.writer(w_mst2)
                writer.writerow(['Time', 'Trial_Number', 'Condition', 'Fixation', 'Fixation_Duration',                          'Stim_ID', 'Subject_Response', 'Accuracy', 'Reaction_Time'])
        ...
        if MST2:
            ...
            for idx in mst2_trials:
                ...
                # display stimulus and record its onset
                MSTtext = visual.TextStim(win, text='Old, Similar, or New?', font=sans, name='instruction',               pos=(0, .4), height=float(.04), wrapWidth=1100, color='black')
        ...
        if ES4:
            ...
            if prevRuns:
                ...
                ES4_csvFile = rootLog + '/sub-' + expInfo['ID'] + '_ses-' + expInfo['session'] + task_lab +                   '_run-01.csv'
        ...
        if RS:
            ...
            RS_FixCross = visual.TextStim(win, name='RS fixation cross', text='+', font=sans, pos=(0, 0),                     height=float(.16), color='gray')
            RS_FixCross.draw()
            win.flip()
        ...
        if ES5:
            ...
            if prevRuns:
                ...
                ES5_csvFile = rootLog + '/sub-' + expInfo['ID'] + '_ses-' + expInfo['session'] + task_lab +                   '_run-01.csv'            
