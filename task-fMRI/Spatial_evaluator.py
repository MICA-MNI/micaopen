...
#############################################################################################################################################################################################
csv_file = ("tasks/spatial/data_Spatial Score (symbolic)/" + expInfo['subject name'] + "_" + expInfo['session'] + "_" + expInfo['date'] + "_" +  expInfo['symbol list'] + "_SCORE" + ".csv")
...
dirfiles = os.listdir('tasks/spatial/data_Spatial Task (symbolic)/')

#############################################################################################################################################################################################
targetfile = [z for z in dirfiles if z.startswith(expInfo['subject name'] + '_' + expInfo['session']) and z.endswith('.csv')]
log_file = 'tasks/spatial/data_Spatial Task (symbolic)/' + targetfile[0]
