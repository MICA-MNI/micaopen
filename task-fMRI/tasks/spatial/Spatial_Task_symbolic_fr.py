...
###################################################################################################################################################################################
expName = "tâche spatiale de changement conformationnel"
...
###################################################################################################################################################################################
if not os.path.isdir('tasks/spatial/data'):
    os.makedirs('tasks/spatial/data')

filename = 'tasks/spatial/data/' + os.path.sep + '%s_%s' %(expInfo['nom du sujet'], expInfo['date'])
...
###################################################################################################################################################################################
csv_file = ("tasks/spatial/data/" + expInfo['nom du sujet'] + "_" + expInfo['session'] + "_" + expInfo['date'] + "_" +  expInfo['liste symbolique'] + ".csv")
...
def instruction(inst_txt='tasks/spatial/Instructions/exp_instr_symbolic_fr.txt'):
...
instruction(inst_txt='tasks/spatial/Instructions/exp_instr_symbolic_fr.txt')
...
########################################################################
instrTxt.setText(open('tasks/spatial/Instructions/wait_trigger_fr.txt', 'r').read())
instrTxt.draw()
win.flip()
core.wait(2)
...
if expInfo['liste symbolique']=='A':
    trialList = data.importConditions("tasks/spatial/Spatial_symbolic/Spatial_ListA.csv")
elif expInfo['liste symbolique']=='B':
    trialList = data.importConditions("tasks/spatial/Spatial_symbolic/Spatial_ListB.csv")
elif expInfo['liste symbolique']=='démo':
    trialList = data.importConditions("tasks/spatial/Spatial_symbolic/Spatial_Listdemo.csv")

LIST = range(len(trialList))

triplet_list = list(zip(LIST,LIST,LIST))
...
########################################################################
uber_list = list(zip(item_1, item_2, item_3, t_positions, E_D, num))
...
def end(end_txt='tasks/spatial/Instructions/end_instr_fr.txt'):
...
end(end_txt='tasks/spatial/Instructions/end_instr_fr.txt')
