...
def get_parameters(skip_gui=False):
    ...
    try:#try to get a previous parameters file 
        ...
        param_settings = [001,2.0,0,'Phase 1','1','2','3','4',False,False,32,1,-1]
...
def show_study(params,study_list,study_cond,set_bins):
    ...
    instructions1=visual.TextStim(win,text="In this task, you will be shown one item at a time. \r\n\r\nYou must choose whether the item belongs indoors or outdoors.\
                                            \r\nMake your choice as fast and as accurately as possible by pressing the button ONLY ONCE. \r\n\r\nTo choose indoors: press '2'\
                                            To choose outdoors: press '4'",pos=(0,0.3),
    ...
    instructions2=visual.TextStim(win,text="Press any button to begin.",pos=(0,-0.6),
        color=(-1,-1,-1),wrapWidth=1.75,alignHoriz='center',alignVert='center')
    instructions3=visual.TextStim(win,text="Indoors or Outdoors?",pos=(0,0.9),
        color=(-1,-1,-1),wrapWidth=1.75)
    
    instructions1.draw()
    instructions2.draw()
    win.flip()
    #key = event.waitKeys(keyList=['space','5','esc','escape'])
    key = event.waitKeys(keyList=['2','3','4'])
    ...
    ####################################################################
    instrTxt = visual.TextStim(win, text='Ready...', pos=(0,0), color=(-1,-1,-1))
    instrTxt.draw()
    win.flip()
    core.wait(2)
    
    fixation = visual.TextStim(win, name='fixation',text='+', font=sans, height=float(0.18), color='black')
    fixation.draw()
    win.flip()
    core.wait(0.5)
    ####################################################################
    ...
    for trial in range(len(study_list)):
		...
		################################################################
        #isi = params['ISI']
        isi = params['ISI'] + random.choice(range(0,6))*0.01
        #isi = range(5,16)[randint(0,11)]*0.1
        ################################################################
...
def show_test(params,test_list,test_cond,set_bins):
    ...
    if params['TwoChoice']==True:
        ...
        instructions1=visual.TextStim(win,text="In this task, you will once again be shown one item at a time. \r\n\r\nThese items may either be the old ones that you saw in the previous task, similar ones, or entirely new ones.\
                                            \r\n\r\nYour task is to determine whether the item is old, similar, or new.\r\n\r\nTo choose Old: press '2'\r\nTo choose Similar: press '3'\
                                            To choose New: press '4'",pos=(0,0.2),
        color=(-1,-1,-1),wrapWidth=1.75,alignHoriz='center',alignVert='center')
    
    #instructions2=visual.TextStim(win,text="Press the spacebar to begin",pos=(0,-0.25),
    instructions2=visual.TextStim(win,text="Press any button to begin",pos=(0,-0.7),
        color=(-1,-1,-1),wrapWidth=1.75,alignHoriz='center',alignVert='center')
    instructions3=visual.TextStim(win,text="Old, Similar, New?",pos=(0,0.9), color=(-1,-1,-1),wrapWidth=1.75)
    ...
    for trial in range(len(test_list)):
        ...
        if RT > 0.05: # We have a response
            ...
            TLF_response_matrix[response-2,trial_type-2] += 1  # Increment the response x type  matrix as needed
            if params['TwoChoice']==True:  # Old/new variant
                ...
                if trial_type == 1 and response == 2:  # Hit
                    correct=1
                elif trial_type == 2 and response == 3:  #Lure-Similar
                    correct=1
                elif trial_type == 3 and response == 4:  #CR
                    correct=1
