...
###################################################################################################################################################################################
if not os.path.isdir('tasks/semantic/data'):
    os.makedirs('tasks/semantic/data')

filename = 'tasks/semantic/data/' + os.path.sep + '%s_%s' %(expInfo['nom du sujet'], expInfo['date'])
...
###################################################################################################################################################################################
csv_file = ("tasks/semantic/data/" + expInfo['nom du sujet'] + "_" + expInfo['session'] + "_" + expInfo['date'] + "_" +  expInfo['liste symbolique'] + ".csv")
...
if expInfo['liste symbolique'] == 'A':
    trialList = data.importConditions("tasks/semantic/Semantic_symbolic/Semantic_ListA.csv")
elif expInfo['liste symbolique'] == 'B':
    trialList = data.importConditions("tasks/semantic/Semantic_symbolic/Semantic_ListB.csv")
elif expInfo['liste symbolique'] == 'démo':
    trialList = data.importConditions("tasks/semantic/Semantic_symbolic/Semantic_Listdemo.csv")
    
LIST = range(len(trialList))

quintuplet_list = list(zip(LIST,LIST,LIST,LIST,LIST))
...
#
########################################################################
uber_list = list(zip(item_1, item_2, item_3, item_4, item_5, num))
