...
if not os.path.isdir('tasks/episodic/data_encoding'):
    print("ERROR: must run 'encoding' first")
    core.quit()
...
filename = 'tasks/episodic/data_retrieval/' + os.path.sep + '%s_%s' %(expInfo['subject name'], expInfo['date'])
...
###################################################################################################################################################################################
csv_file = ("tasks/episodic/data_retrieval/" + expInfo['subject name'] + "_" + expInfo['session'] + "_" + expInfo['date'] + "_" +  expInfo['symbol list'] + ".csv")
...
def instruction(inst_txt='tasks/episodic/Instructions/exp_instr2_symbolic.txt'):
...
instruction(inst_txt='tasks/episodic/Instructions/exp_instr2_symbolic.txt')
...
########################################################################
instrTxt.setText(open('tasks/episodic/Instructions/wait_trigger.txt', 'r').read())
instrTxt.draw()
win.flip()
core.wait(2)
...
x_values = [-0.5,0,0.5] # the y value for the target images is always the same (i.e., -0.4); only the x values must be specified
...
def end(end_txt='tasks/episodic/Instructions/end_instr.txt'):
...
end(end_txt='tasks/episodic/Instructions/end_instr.txt')
