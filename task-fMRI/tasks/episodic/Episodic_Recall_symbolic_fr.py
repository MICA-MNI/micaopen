...
if not os.path.isdir('tasks/episodic/data_encoding'):
    print("ERREUR: exécutez l'étape << encodage >> en premier")
    core.quit()
...
expName = "rappel épisodique"
expInfo = {'nom du sujet':'', 'session':'001', 'liste symbolique':['A','B','démo']}
...
if not os.path.isdir('tasks/episodic/data_retrieval'):
    os.makedirs('tasks/episodic/data_retrieval')

filename = 'tasks/episodic/data_retrieval/' + os.path.sep + '%s_%s' %(expInfo['nom du sujet'], expInfo['date'])
...
###################################################################################################################################################################################
csv_file = ("tasks/episodic/data_retrieval/" + expInfo['nom du sujet'] + "_" + expInfo['session'] + "_" + expInfo['date'] + "_" +  expInfo['liste symbolique'] + ".csv")
...
MR_settings = {
'TR': 0.6, # duration (sec) per volume
...
#################################################################################################################################################################
if expInfo['liste symbolique'] == 'A':
    trialList = data.importConditions("tasks/episodic/Episodic_Encoding_symbolic/Episodic_Recall_ListA.csv")
elif expInfo['liste symbolique'] == 'B':
    trialList = data.importConditions("tasks/episodic/Episodic_Encoding_symbolic/Episodic_Recall_ListB.csv")
elif expInfo['liste symbolique'] == 'démo':
    trialList = data.importConditions("tasks/episodic/Episodic_Encoding_symbolic/Episodic_Recall_Listdemo.csv")

LIST = range(len(trialList))

#############################################################################################################################
dirfiles = os.listdir('tasks/episodic/data_encoding/')
targetfile = [z for z in dirfiles if z.startswith(expInfo['nom du sujet'] + '_' + expInfo['session']) and z.endswith('.csv')] 

if expInfo['liste symbolique'] == 'démo':
    main_order = range(0,4)
elif expInfo['liste symbolique'] != 'démo':
    main_order = range(0,56)

encoded_log_file = 'tasks/episodic/data_encoding/' + targetfile[0]
...
num = range(0,56)
uber_list = list(zip(LIST, LIST, LIST, LIST, num))
