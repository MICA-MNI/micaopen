...
#
def instruction(intro_txt='tasks/MW/Instructions/mind_wandering_instr.txt'):
...
#
instruction(intro_txt='tasks/MW/Instructions/mind_wandering_instr.txt')
...
waitTxt.setText(open('tasks/MW/Instructions/wait_trigger.txt', 'r').read())
waitTxt.draw()
win.flip()
core.wait(2)
...
#######################################################################
trialList = data.importConditions('tasks/MW/mind_wandering_trials.csv')
#
LIST = range(1,13)
shuffle(LIST)
...
for x,y in main_list:
	...
	#
	if trialList[x]['questions'].startswith('The'):
		rating_scale = visual.RatingScale(win, scale=None, labels=['negative', 'positive'], high=11, markerStart=6, leftKeys='2', rightKeys='4',
	...
	elif trialList[x]['questions'].endswith('were:'):
		rating_scale = visual.RatingScale(win, scale=None, labels=['spontaneous', 'deliberate'], high=11, markerStart=6, leftKeys='2', rightKeys='4',
		tickMarks=['1','11'], tickHeight=1, maxTime=7.5, markerColor='black', textColor='black', textSize=0.75, stretch=2.5, noMouse=True, lineColor='#4CB391',
		marker='slider', showValue=False, precision=5, showAccept=False)	
	else:
		rating_scale = visual.RatingScale(win, scale=None, labels=['not at all', 'completely'], high=11, markerStart=6, leftKeys='2', rightKeys='4',
		tickMarks=['1','11'], tickHeight=1, maxTime=7.5, markerColor='black', textColor='black', textSize=0.75, stretch=2.5, noMouse=True, lineColor='#4CB391',
		marker='slider', showValue=False, precision=5, showAccept=False)
	#
...
#
def end(end_txt='tasks/MW/Instructions/mind_wandering_end.txt'):
    ...
    for i, cur in enumerate(End):
        ...
        core.quit()
#
end(end_txt='tasks/MW/Instructions/mind_wandering_end.txt')
