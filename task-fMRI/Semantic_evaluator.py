...
#############################################################################################################################################################################################
csv_file = ("tasks/semantic/data_Semantic Score (symbolic)/" + expInfo['subject name'] + "_" + expInfo['session'] + "_" + expInfo['date'] + "_" +  expInfo['symbol list'] + "_SCORE" + ".csv")
...
dirfiles = os.listdir('tasks/semantic/data_Semantic Task (symbolic)/')

#############################################################################################################################################################################################
targetfile = [z for z in dirfiles if z.startswith(expInfo['subject name'] + '_' + expInfo['session']) and z.endswith('.csv')]
log_file = 'tasks/semantic/data_Semantic Task (symbolic)/' + targetfile[0]
