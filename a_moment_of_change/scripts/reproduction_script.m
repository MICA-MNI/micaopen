...
for init_project = 1
    ...
    perm_id = csvread([baseDir '/perm_sphere_10000.csv']);
...
for Figure1 = 1
    ...
    corr(MTmoments_bl', 'type', 'spearman') % cross-correspondence of MT moments, showing unique but related information
    p_perm = perm_sphere_p(MTmoments_bl(1,:)', MTmoments_bl(3,:)', perm_id)
...
% Figure 1B&C- baseline MT moments
for Figure1A = 1
...
for Figure_1D = 1
    ...
    corr(embedding(:,1), MTmoments_bl(1,:)', 'type', 'spearman')
    p_perm = perm_sphere_p(embedding(:,1), MTmoments_bl(1,:)', perm_id)
    ...
    corr(embedding(:,1), MTmoments_bl(3,:)', 'type', 'spearman')
    p_perm = perm_sphere_p(embedding(:,1), MTmoments_bl(3,:)', perm_id)
...
%% FIGURE 2 
% Figure 2A - exemplar profiles for ROIS
for Figure2A = 1
...
for correction_and_figure = 1
    ...
    for t = 1:2
        ...
        % calculate average MT moments for mesulam classes
...
%% Supplement - association MT moments with laminar thickness
for correlation_and_figure = 1
    
    load([baseDir '/laminar_thickness_sjh.mat'], 'laminar_thickness')

    thick_corr = corr([laminar_thickness' MTmoments_bl']);
    csvwrite([outDir 'laminar_thickness_corr.csv'], thick_corr)
    tmp = thick_corr(7:end,1:6);
    min(tmp(:))
    max(tmp(:))

    for Supp_figure_laminar = 1
        
        f = figure('units','centimeters','outerposition',[0 0 30 30]);
        for l = 1:6
            
            surface_map = BoSurfStatMakeParcelData(laminar_thickness(l,:), FS, parc);
            surface_map = SurfStatSmooth(surface_map, FS, 5);
            clim = [min(laminar_thickness(l,:)) max(laminar_thickness(l,:))]; % set colour limits
            surface_map = surface_map .* mask';
            BoSurfStat_calibrate4Views(surface_map, FS, ...
                [(l*0.16)-0.16 0.86 0.08 0.08; (l*0.16)-0.085 0.78 0.08 0.08; ...
                (l*0.16)-0.16 0.78 0.08 0.08; (l*0.16)-0.085 0.86 0.08 0.08], ...
                1:4, round(clim, 2), inferno)
            hbar = colorbar('horiz', 'Ticks', round(clim, 2));
            hbar.Position = [(l*0.16)-0.115 0.77 0.07 0.01];
            
        end
    
        exportfigbo(f, strcat(figDir, '/laminar_thickness.png'), 'png', 10)
    end
...
for Figure4 = 1
    ...
    for Figure4A = 1
       ...
       for Figue_4C = 1 
           ...
           set(gca, 'YTickLabel', [])
           set(gca, 'XTickLabel', [])
           corr(fig2b_maps(1,:)', anchor_diff', 'type', 'spearman')
           perm_sphere_p(fig2b_maps(1,:)', anchor_diff', perm_id)
           ...
           set(gca, 'YTickLabel', [])
           set(gca, 'XTickLabel', [])
           corr(fig2b_maps(3,:)', anchor_diff')
           perm_sphere_p(fig2b_maps(3,:)', anchor_diff', perm_id)
           ...
           set(gca, 'YTickLabel', [])
           set(gca, 'XTickLabel', [])
           corr(fig2b_maps(3,:)', anchor_diff', 'type', 'spearman')
           perm_sphere_p(fig2b_maps(3,:)', anchor_diff', perm_id)
