"""
Conjugate Gradient for Rotations manifold. Adapted from Pymanopt
"""
...
from SFlinesearch import LineSearchAdaptive
...
class SFConjugateGradient(Solver):
    """
    ...
    def __init__(self, beta_type=BetaTypes.HestenesStiefel, orth_value=np.inf,
                 linesearch=None, cost_fun=None, grad_fun=None,
                 *args, **kwargs):
        """
    ...
    def solve(self, problem, x=None, reuselinesearch=False):
        ...
        #######################################################################
        #######################################################################
        cost, store = self.cost_fun(x, return_store=True)
        grad = self.grad_fun(x, store=store)

        #######################################################################
        #######################################################################

        gradPgrad = (grad**2).sum()
        gradnorm = np.sqrt(gradPgrad)
        Pgrad = grad
        ...
        while True:
            ...
            ###################################################################
            ###################################################################
            newcost, newstore = self.cost_fun(newx, return_store=True)
            newgrad = self.grad_fun(newx, store=newstore)

            ###################################################################
            ###################################################################
            newgradPnewgrad = (newgrad**2).sum()
            newgradnorm = np.sqrt(newgradPnewgrad)
            Pnewgrad = newgrad

            # Apply the CG scheme to compute the next search direction
            oldgrad = grad.copy()
            grad *= newgrad
            orth_grads = grad.sum() / newgradPnewgrad
